...
public enum SleepStager {
    ...
    // only when that coverage reaches maxOffWristSleepFraction of its duration, so a real night that
    // over-extends into a SHORT off-wrist tail survives. This is independent of the daytime band —
    // off-wrist time is off-wrist day or night, and a night-tail continuation does NOT exempt it.
    ...
    /// A candidate sleep run is dropped only when its off-wrist coverage reaches this fraction.
    public static let maxOffWristSleepFraction: Double = 0.5

    /// Only trust the HR-gap off-wrist proxy when the HR stream is dense enough that a long gap is anomalous.
    static let hrDenseSpacingS: Int = 600
    ...
    /// LARGEST gravity inter-sample gap exceeds maxGapMin. Requires a real HR span to compare against —
    /// with no/degenerate HR the dense path is kept (false), so a 4.0 with absent HR is never
    /// reclassified as sparse.
    ...
    /// Off-wrist HR-gap spans within [p.start, p.end].
    ...
    /// Fractional off-wrist coverage of a candidate run [p.start, p.end] in [0, 1].
    ...
    /// the strap's WRIST_OFF/WRIST_ON events. When the call site has them, they sharpen the always-on
    /// HR-gap off-wrist backstop.
    public static func detectSleep(hr: [HRSample] = [],
        ...
        for p in runs {
            ...
            // Off-wrist backstop: drop only when off-wrist coverage is substantial, so a short off-wrist
            // tail on an otherwise real night survives.
